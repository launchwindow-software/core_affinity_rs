//! FreeBSD CPU affinity implementation.
...
/// Returns core ids available to the current thread's FreeBSD affinity mask.
...
/// Attempts to pin the current thread to the given FreeBSD core id.
...
mod tests {
    ...
    fn test_freebsd_get_affinity_mask() {
        match get_affinity_mask() {
            Some(_) => {}
            None => {
                panic!("freebsd::get_affinity_mask() returned None");
            }
        }
    }
    ...
    fn test_freebsd_get_core_ids() {
        match get_core_ids() {
            ...
            None => {
                panic!("freebsd::get_core_ids() returned None");
            }
        }
    }
